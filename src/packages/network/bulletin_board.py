...
def popup(auth_no):
    ...
    label = tk.Label(popup, text=f"AUTHORITY {auth_no} PROVIDED AN INCORRECT DECRYPTION PROOF.", fg="red", font=("Helvetica", 20))
    label.pack(pady=20)
...
def setup_group_params(
    bits, predifined_group_id=None
):  # before pub key available to vote
...
def aggregate_votes(cipher=AddElGamal):
    ...
    files = [
        ...
        if os.path.isfile(os.path.join(votes_directory, file))
    ]
...
def get_lagrange_coeff(index, index_list, cipher=AddElGamal):
    ...
    for i in index_list:
        ...
        denominator = gmp.f_mod(
            denominator * gmp.f_mod((i - index), cipher.q), cipher.q
        )
...
def tally_votes(threshold, cipher=AddElGamal):
    ...
    correct_decryptions = get_decryption_with_correct_proof(
        total_votes, threshold, cipher
    )
    if correct_decryptions == None:
        return
    ...
    for decryption in correct_decryptions:
        decrypted_value = base_k_exp(
            base64_to_int(decryption["DECRYPTED_VOTE"]),
            get_lagrange_coeff(decryption["AUTH_NO"], index_list, cipher),
            cipher.modulus,
            K,
        )
    ...
    decrypted_votes = int(
        gmp.f_mod(
            gmp.mul(total_votes[1], gmp.invert(denominator, cipher.modulus)),
            cipher.modulus,
        )
    )
            ...
            while True:
                try:
                    ...
                    thread = threading.Thread(
                        target=handle_authority, args=(conn, encryptions)
                    )
                    thread.start()
