...
class PublicSecretSharing:
    ...
    def distribute_secret(self, participants_key, threshold, secret):
        ...
        coefficients = [
            randbelow(self.q) for _ in range(threshold - 1)
        ]  # t-1 coefficients.
        ...
        for participant_index, key in enumerate(participants_key):
            encryptions.append(
                (
                    participant_index,
                    base_k_exp(key, poly(participant_index + 1), self.p, K),
                )
            )
        ...
        for i in range(participants_number):
            ...
            for j in range(threshold):
                C_j = base_k_exp(
                    coefficient_commitments[j], pow(i + 1, exp, self.q), self.p, K
                )
    ...
    def decrypt_and_share(self, share, pub_key, priv_key):
        ...
        proof = dlog_eq(
            self.beta, pub_key, decrypted_share, share, self.p, self.q, priv_key
        )
        ...
        for i in range(0, len(shares)):
            ...
            for j in range(0, len(shares)):
                ...
                denominator = gmp.f_mod(
                    gmp.mul(
                        gmp.f_mod(shares[j][0] - shares[i][0], self.q), denominator
                    ),
                    self.q,
                )

            exponent = int(
                gmp.f_mod(gmp.mul(numerator, gmp.invert(denominator, self.q)), self.q)
            )
...
class VerifiableSecretSharing:
    ...
    def distribute_secret(self, participants_key, secret):
        ...
        # Generate polynomial coefficients and create the polynomial.
        coefficients = [
            randbelow(self.q) for _ in range(self.threshold - 1)
        ]  # t-1 coefficients.
        coefficients.append(secret)  # Smallest order coefficient is the secret.
        poly = Modular_Polynomial(coefficients, self.q)
        ...
        coefficient_commitments = []
        for coeff in coefficients:
            coefficient_commitments.append(
                int(base_k_exp(self.generator, coeff, self.p, K))
            )
